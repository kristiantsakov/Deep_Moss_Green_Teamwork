...
public class Game extends JPanel implements KeyListener, ActionListener,
		Runnable {
	// movement keys..
	public static final String BACKGROUND_IMG_NAME = "bgr.jpeg";
	...
	public void paint(Graphics g) {
		ImageIcon ib = new ImageIcon(BACKGROUND_IMG_NAME);
		g.drawImage(ib.getImage(),0,0,null);
		g.fillRect(0, 0, 800, 600);

		// Drawing Ball
		ImageIcon ic = new ImageIcon(getClass()
				.getResource("/images/ball.png"));
		...
		if (ballFallDown == true || bricksOver == true) {
			...
			g.drawString(finalScore, 440, 120);
			ballFallDown = false;
			bricksOver = false;
		}
	...
	public void run() {
		...
		for (int i = 0; i < Brick.length; i++) {
			Brick[i] = new Rectangle(brickx, bricky, 120, 20);
			brickx += 125;
			if (i == 5) {
				brickx = 25;
				bricky = 150;
	...
	public void restart() {
		...
		Bat = new Rectangle(batx, baty, 150, 20);
		// Rectangle Brick;// = new Rectangle(brickx, bricky, 30, 10);
		Brick = new Rectangle[18];
